...
新建user消息会把经过请求边界清理的`POST /v1/chat/runs.metadata`直接保存到现有`metadata_json`，历史接口继续通过`metadataJson`原样返回。该行为覆盖NEXT、EDIT_USER和普通Intent澄清新建的user消息；重新生成、候选技能切换及复用历史消息的Interaction不会覆盖原消息metadata。空metadata仍保存为`null`，`selectedIntent`等服务端内部载体不会进入消息历史。
...
# 可选，默认空：请求逻辑入口 fin -> EX_fin；未传入口仍使用上述默认值，不加前缀
export FINANCEEX_INTENT_REQUEST_ACCESS_NAME_PREFIX=EX_
