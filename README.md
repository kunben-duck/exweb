...
DomainAgent；提交 questionnaire 文本或附件表示“其他”，此时才重新调用 IntentAgent。超时任务使用同一
Interaction CAS 自动选择最高置信度候选，和人工提交最多一个成功。run-A 与 run-B 使用不同 runId，但复用
同一 user/assistant 消息，选择响应和最终回答以不同来源 runId 的 parts 追加到该 assistant。超时任务只保存在
当前实例内存，实例重启后不会接管；Interaction 仍可由用户续接。
