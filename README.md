...
- `GET /v1/chat/sessions/page?appScope=MAIN_SITE&keyword=利润&channel=mobile&curPage=1&pageSize=20`：页码分页查询当前用户历史会话；`keyword`统一模糊匹配标题、已持久化user问题和assistant回答，返回 `totalRows/totalPages` 和每个会话第一条 assistant 的正文及原始 metadata 字符串。搜索事务默认限时2秒，超时返回`503/SESSION_SEARCH_TIMEOUT`。

页码会话关键字搜索及两个会话列表的最后Run状态批量读取，共用`FINANCEEX_SESSION_SEARCH_DATABASE_QUERY_TIMEOUT_SECONDS`数据库事务超时，允许`1..30`秒，默认2秒；非法值会使服务启动失败。关键字搜索超时返回`503/SESSION_SEARCH_TIMEOUT`，最后Run状态读取超时则保持列表成功并返回`lastRunStatus=null`；普通列表主体查询不使用该超时配置。
