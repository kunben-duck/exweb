...
DomainAgent Ask User 使用 `approval-request(operation_type=questionnaire)`，复用现有问卷卡片、Interaction
及 `CONTINUE_INTERACTION`。问卷结束当前 HTTP 流，答案通过同一 chat 地址的 `approval-response` 控制请求续跑，
不重发原 query/附件/messages；原 user/assistant 保持复用。前端自动忽略配置为
`financeex.domain-agent.questionnaire-wait-timeout`（`FINANCEEX_DOMAIN_AGENT_QUESTIONNAIRE_WAIT_TIMEOUT`），
默认 `0s` 关闭，与 Relay 独立且不注册服务端定时任务。下游须支持等待上下文恢复及 `ignore=true`。
协议与失败边界见 [DomainAgent Ask User](docs/domain.md#64ask-user-问卷与答案续跑)。
