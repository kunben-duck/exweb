...
- 意图服务 adapter 的 HTTP 请求体和响应体转换由 infrastructure intent mapper 承载。`financeex.intent.invocation-mode` 默认 `BLOCKING` 并调用 `/getIntentDecision`；设为 `STREAMING` 时只调用 `/getIntentDecisionStream`，不按 Content-Type 自动切换接口。两种模式使用相同请求 mapper 和结果 mapper，均以完整响应中的 `data.result.routeAction` 作为唯一裁决点。
- SSE 模式只消费 `progress/delta/result/error`。`progress` 和 `delta` 分别映射为 `runtime.progress(sourceType=intent-progress)` 与 `runtime.thinking(sourceType=intent-delta)`，按原顺序进入事件事实表和实时通道；ping 注释不生成事件，只有 `result` 驱动路由。首业务事件、网络空闲和单次尝试总时长分别受独立超时控制；SSE error、HTTP 错误、异常断流和协议错误仍使用现有重试与 failure strategy。
...
- 意图服务：`financeex.intent.enabled`、`base-url`、`access-name`、`response-access-name-prefix`、`no-match-agent-name`、`invocation-mode`、`recognize-path`、`recognize-stream-path`、`trace`、`confidence-threshold`、`timeout`、`stream-first-event-timeout`、`stream-idle-timeout`、`stream-total-timeout`、`max-retries`、`failure-strategy`。`invocation-mode` 只接受 `BLOCKING/STREAMING` 且默认 `BLOCKING`；请求侧 `access-name` 是意图入口；可选的 `response-access-name-prefix` 仅用于把响应 `items[].accessName` 归一化为真实 DomainAgent skillId；`no-match-agent-name` 仅控制 `NO_MATCH` 的展示名称，默认 `FIN Supervisor Agent`；`failure-strategy` 只接受 `RELAY_FALLBACK/FAIL_RUN`；`confidence-threshold` 仅保留给旧统计字段兼容，不参与 DomainAgent 路由裁决。
