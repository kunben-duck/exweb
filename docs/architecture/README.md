...
GET  /v1/chat/sessions/page?appId=...&title=...&channel=mobile&curPage=1&pageSize=20
...
会话表以显式 `app_id/app_name` 保存产品分组标签：`appId` 是大小写敏感的稳定查询键，`appName` 是创建时展示快照。两者不参与身份隔离，所有读取仍必须带 `tenantId + userId`；`/sessions/apps` 使用单条轻量窗口查询返回非删除会话中的去重分类，名称取最近非空快照，排序使用各分类最新会话时间，不读取消息或完整会话字段。列表可按 `appId` 精确过滤，并按 `title` 执行大小写不敏感的包含搜索，两个条件同时存在时使用 `AND`。可选 `channel` 与上述条件继续使用 `AND`，用于移动端创建和列表展示隔离：移动端统一传 `mobile`，PC 端省略后仍查看全部渠道；它不替代 `tenantId + userId` 权限边界。游标绑定规范化后的过滤条件，不能在后续页切换 `appId/title/channel`；无 channel 的既有 v2/v3 游标继续兼容，带 channel 的查询使用 v4 游标。`%/_/!` 在应用层转义为普通字符，SQL 使用参数化 `ILIKE ... ESCAPE '!'`，不拼接用户输入。现有 `(tenant_id,user_id,app_id,updated_at,id)` 索引继续用于归属、分类和 appId 范围收敛，不增加 DDL。已有会话只接受与快照一致的显式 tag 和显式 channel，分支继承源字段，其他会话更新不修改它们。字段保留在会话边界，不进入 run metadata、RouteMemory 或 Agent 请求，也不增加主流程外部调用。
...
- 意图服务返回 `routeAction=ROUTE_SINGLE` 时，取唯一 `items[0].accessName`，按可选 `response-access-name-prefix` 移除一次匹配的通用前缀，再区分大小写匹配显式配置的 `domain-expert-access-name-prefix`。命中专家前缀时再移除一次该前缀并 trim，剩余后缀作为 Relay `DOMAIN_EXPERT` 的动态 `roleName`；未命中时规范化值作为 DomainAgentId/skillId 绑定并调用 DomainAgent Runtime。前端直连同名 DomainAgent 不触发专家转换。调用 Runtime 前，服务端以本轮可信附件的完整 `providerDocument` 覆盖 `sceneParam.docList`；`ROUTE_MULTI/NO_MATCH/RELAY_FALLBACK` 仍进入 Relay `DELEGATE`。`NO_MATCH` 的展示名称及失败策略保持不变；缺少 item、有效 `accessName` 或专家角色后缀均是协议失败。
...
- 意图服务：`financeex.intent.enabled`、`base-url`、`access-name`、`response-access-name-prefix`、`domain-expert-access-name-prefix`、`no-match-agent-name`、`invocation-mode`、`recognize-path`、`recognize-stream-path`、`trace`、`confidence-threshold`、`timeout`、`stream-first-event-timeout`、`stream-idle-timeout`、`stream-total-timeout`、`stream-auth-timeout`、`stream-auth-io-max-size`、`stream-auth-io-queue-capacity`、`ambiguous-route-wait-timeout`、`max-retries`、`failure-strategy`。`invocation-mode` 只接受 `BLOCKING/STREAMING` 且默认 `STREAMING`；可选的 `response-access-name-prefix` 先归一化候选，随后显式且非空的 `domain-expert-access-name-prefix` 以区分大小写的前缀匹配选择 Relay 专家模式，移除前缀后的后缀直接作为动态 `roleName`；意图功能开启但专家前缀未配置时启动失败。`ambiguous-route-wait-timeout` 默认 `30s`，同时控制 AMBIGUOUS_ROUTE 代选与拒答路由切换确认的前端截止时间；其余重试和展示配置保持原语义。
