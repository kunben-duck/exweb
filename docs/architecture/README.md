...
会话表以显式 `app_id/app_name` 保存产品分组标签：`appId` 是大小写敏感的稳定查询键，`appName` 是创建时展示快照；`app_id IS NULL`定义为主站会话。两者不参与身份隔离，所有读取仍必须带 `tenantId + userId`；`/sessions/apps` 使用单条轻量窗口查询返回非删除会话中的去重分类，仍排除主站会话。列表可按 `appId` 精确过滤，或使用互斥的`appScope=MAIN_SITE`查询主站；两者均可与大小写不敏感的`title`包含搜索及精确`channel`过滤组合。省略`appScope/appId`保持查询全量，PC现有行为不变。游标绑定全部过滤条件，主站查询使用v5，既有v2/v3/v4继续兼容。`%/_/!` 在应用层转义为普通字符，SQL 使用参数化 `ILIKE ... ESCAPE '!'`，不拼接用户输入。现有索引继续用于归属和范围收敛，不增加DDL。已有会话只接受与快照一致的显式tag和channel，分支继承源字段；这些字段不进入run metadata、RouteMemory或Agent请求。
