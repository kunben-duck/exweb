...
基于当前实现的场景服务级时序图、风险登记、加固任务、高可用测试与故障演练，参见
...
- DomainAgent 异步任务：可选协议开启后，`agent.async_started` 将原 HTTP 流转换为 `ASYNC_WAITING` 控制面状态；业务 run 继续保持 `RUNNING`，后续由携带可信 `runId` 的内部回调提交完成或失败，并可按APPEND/REPLACE回填原始DomainAgent帧。回调入口在JSON反序列化前执行并发和请求体保护；快速回调以可重试409等待状态提交，终态CAS校验异步租约，Binding使用expected run条件更新。标准化与容量校验在终态事务前完成，最多128帧、128个业务事件和1MiB事件数据；纯终态帧不触发REPLACE，拒答等状态机控制帧不进入回调终态。FULL结果Event、assistant正文、当前run Parts和终态在同一事务提交；no-store业务结果只实时推送。
...
- DomainAgent 技能配置明确返回 `isSaveSession=N` 时，本轮策略收紧为 `ASSISTANT_PLACEHOLDER`：普通 `message.*` 与下游 `runtime.*` 业务 Event 只分配全局 sequence 并经本机/Redis 实时发布，不进入事件表；Intent、路由、拒答、WAIT、确认和 run 终态仍持久化。`stream-status.latestSeq` 只表示最新持久化位置，Event Resume 不补发 live-only 业务内容。同一 run 后续切到 Relay 也不会放宽策略，普通 Relay run 仍使用 `FULL`。留存策略和附件类型校验共享完整技能配置缓存；`financeex.domain-agent-skill-config.cache-enabled=false` 时每次新的配置解析都绕过 Redis 实时查询 Provider，旧 `FINANCEEX_AGENT_DATA_PERSISTENCE_CACHE_ENABLED` 环境变量仅作为新配置未设置时的兼容回退。Interaction continuation 仍继承 source run 已固化的留存策略。
...
- DomainAgent独立的`type=recommended_questions`帧在`recommendedQuestions`为数组时映射为`runtime.card(sourceType=recommended_questions,cardType=recommendedQuestions,cardSources=[recommendedQuestions])`，完整保留推荐问及下游`conv_id/run_id/seq`业务字段；下游标识不参与ChatService的run或sequence控制。`openCard`同帧携带推荐问时仍保持原卡片分类，不将推荐问加入`cardSources`。
...
- `cache-recent-turns=5` 只限制 Redis 容量，按每轮最多 user/assistant 两条消息保存紧凑条目。条目不包含 Parts、附件、完整 metadata 或重复的归属字段，只额外保留可选的规范化 `skillId` 以保证缓存命中与数据库回源的下游历史一致。
- `agent-runtime.recent-turns=5` 与 `agent-runtime.max-context-tokens=4096` 独立控制 Relay 和 DomainAgent 的 user/assistant 历史。Relay 只在普通 `user-message.messages` 中发送；DomainAgent 在请求根节点 `messages` 中发送，并覆盖前端 metadata 的同名字段。assistant使用自身服务端metadata中的单值`skillId`，同轮user使用当前路径直接子assistant的相同标识；未知时省略。控制帧不携带历史。
