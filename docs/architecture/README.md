...
- 下游尽力取消：Relay stream-http 通过 HTTP stopPath 取消，Relay WebSocket 通过当前下游连接发送 `{"type":"interrupt"}`，SubAgent 走各自 cancel adapter；这些下游取消失败只记录日志，不影响前端收到本服务 `run.cancelled` 终态。
...
- Relay WebSocket adapter：`financeex.agent-runtime.relay.websocket.url`、`app-mode`、`connection-mode`、`connect-timeout`、`config-handshake-timeout`、`idle-timeout`、`interrupt-pause-timeout`、`max-frame-bytes`、`idle-ttl`、`max-cached-connections`
...
SubAgent 当前只支持单轮 HTTP 文本流调用。当前上线版本内置一个 `RelayAgentRuntime` provider，并通过 `RelayRuntimeProtocolAdapter` 在 provider 内选择下游协议：`relay-stream-http` 是默认 Relay HTTP 流式协议实现，`relay-websocket` 是可选 Relay WebSocket 普通问答实现。Relay WebSocket 支持 `short` 与 `single-instance-reuse` 两种连接模式：默认短连接每个 run 建连，单实例复用模式只在当前 JVM 内按 ChatService 会话缓存连接，不做跨实例连接路由。新增下游协议时，应新增 adapter，而不是在 `RelayAgentRuntime` 主类里堆转换分支。
