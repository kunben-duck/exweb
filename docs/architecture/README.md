...
- DomainAgent 异步任务：可选协议开启后，`agent.async_started` 将原 HTTP 流转换为 `ASYNC_WAITING` 控制面状态；业务 run 继续保持 `RUNNING`，后续由携带可信 `runId` 的内部回调提交结果或终态。
