...
| CN09配置、目标N09/N11/N16 | 映射/留存配置过期或版本不一致；R36 → W05/W11 → T36 |
| N17/N18/N19 | 500MiB传输、元数据登记与对象结果分离；R12 → W06 → T12 → RB07/D06 |
| CN02/CN08、N02/N08 | 路由、鉴权头、流缓冲、期限和重试错误跨服务传播；R32 → W12 → T32 |
| N03–N05 | 静态源故障；R31 → W12 → T31 → RB12/D10 |
| CN12/CN13/CN10及目标对应节点 | DomainAgent、intentService、relayService不可用、断流与等待放大；R39/R40/R41 → W05/W07 → T39/T40/T41 → RB05/D04。切区不能修复共同第三方故障 |
...
发布清单含releaseId、内容哈希、HTML入口、带内容哈希的静态资源、运行配置、适配后端版本和回滚版本。先上传资源并验证，再切入口版本；HTML/运行配置采用能及时更新的缓存策略，带哈希资源长缓存且保留旧版本。禁止仅清缓存而删除仍被旧HTML引用的资源。当前agentService中的动态页面配置（目标adminService）不随静态备用自动恢复，T36/T31同时验证其不可用时的实际体验。
...
步骤01–04/10 → R31/T31；05–09/11–12 → R31/R32/T31/T32；统一W12、RB12/D10。WCM失败但ADS业务正常时优先只切静态源；ALB或ADS运行面失效再按DEP05进行区域级接管。
...
步骤01–04/14 → R33/R35；05–08 → R28/R29/R35/R36；09–13 → R03/R31–R36/R39–R41。主闭环R35 → W14 → T35 → RB14/D12；关联T28/T29/T31–T36及T39–T41覆盖前置能力。
...
步骤01–03/11 → R28/R31/R33/R35/R36；04–10/12–13 → R35，并联合R41取消残留、R37工具执行清理以及R03恢复放量验证；未知外部结果禁止自动重放。闭环W14、T35、RB14/D12。

## 4. 平台能力与证据清单

| 编号 | 必须具备并验证的能力 | 责任与证据 | 缺失时的限制 |
|---|---|---|---|
| CAP01 | ALB跨AZ可用、全部文根一致、内部访问边界、备用HTTPS源接入 | 入口负责人；规则导出、协议与T31/T32报告 | 不宣称入口/WCM容灾可用 |
...
| CAP07 | agentService当前及拆分后toolService映射版本、chat/MCP执行/取消/回调关联，Relay会话和命令顺序 | agentService/relayService及拆分目标服务/第三方负责人；联合契约和T36/T35 | 未知结果不盲重试，不承诺Runtime接管 |
