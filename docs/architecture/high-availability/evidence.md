...
源码基线：`8f48d6cc084be91bcbaad90be43dac7181636cb4`；文档检查日期：2026-09-22（Asia/Shanghai）。本页只维护当前方案的验证状态。当前架构以[部署现状与目标](deployment.md#dep01)为准，风险和测试使用R01–R21、T01–T21。

## 1. 当前检查结果

| 检查 | 结果 | 证明范围 |
|---|---|---|
| 文档结构 | 7份主文档，12个场景、19张主图、21项风险及测试、13个工作包、13份运行册、12组演练 | 当前范围和编号完整，不代表措施已实施 |
| 风险追踪 | R01–R21与T01–T21连续、唯一、一一对应；R→W→T→RB→D关联完整 | 每项风险具备措施、责任角色和验证入口，实际责任人及关闭证据仍须补齐 |
...
| Mermaid | 12张场景图与7张部署图重新渲染为SVG，语法和稳定步骤ID检查通过 | 图源未改变运行语义；不代表依赖故障和容灾测试通过 |
...
- [T01–T21测试规格](tests.md)和[D01–D12演练](operations.md)均为 **NOT_RUN**；没有新的业务测试、混合压测、生产故障注入或容灾验收结果。
...
| 文档与旁路 | 500MiB直传或隔离转发、慢流取消及孤儿对象对账；长历史查询、恢复风暴及旁路混合负载对主链路的影响 |
...
```sh
mkdir -p /tmp/financeex-ha-current
...
本机当前检查产物位于`/tmp/financeex-ha-current/`，包括`check.json`、渲染日志、SVG和`render-manifest.json`；临时文件不保证跨机器存在。正式验收应保存实现版本、有效参数、双方契约、原始日志/指标、资源与业务断言、恢复结果及具名复核；只有文档通过检查不能作为验收结论。
