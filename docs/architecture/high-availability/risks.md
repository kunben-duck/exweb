...
本轮聚焦OOM、CPU、线程/锁停滞、洪峰和依赖失效引起的服务不稳定，收敛为 **22项主风险、13个待实施工作包**。每项按“触发 → 故障传播 → 资源/服务影响 → 已有保护与缺口 → 措施验收”记录。[唯一追踪矩阵](#traceability)连接风险、工作包、测试、运行册和演练；源码细节见本页，调用关系见[场景图](scenarios.md#flows)。

<a id="risk-register"></a>
## 当前风险与证据边界

复核日期 **2026-09-22**；代码基线 `8f48d6cc084be91bcbaad90be43dac7181636cb4`。本轮只修订落地方案，未改业务代码、未执行大文件/OOM或故障注入。默认值不是生产有效配置；容量阈值和真实故障行为均待验证，已执行检查见[证据](evidence.md)。
...
S链接定位[当前场景](scenarios.md)，DEP链接定位[部署方案](deployment.md)。T/RB/D分别定位[测试](tests.md)及[运行册/演练](operations.md)。主责以角色登记，实施前须补具名负责人。
...
| [R22](#r22) 前端WS连接/订阅洪峰 | OPEN+ENV/P1大连接量上线门槛 | [S03](scenarios.md#s03)、[S07](scenarios.md#s07)、[S12](scenarios.md#s12) | [W03](#w03)/[W01](#w01)/[W04](#w04)/[W12](#w12) | 应用、前端、SRE、入口负责人 | [T22](tests.md#t22) | [RB06](operations.md#rb06)/[RB01](operations.md#rb01) | [D05](operations.md#d05) |
...
- **证据 S**：[构造器](../../../src/main/java/com/huawei/it/ex/one/infrastructure/persistence/RedisChatLiveEventBus.java#L73)手工创建 listener，仅设 connectionFactory；当前依赖 Spring Data Redis 3.4.6 的 `createDefaultTaskExecutor` 使用 Spring Core 6.2.7 的 `SimpleAsyncTaskExecutor`，默认 concurrencyLimit=-1，未设置 virtual delegate。源码和本地依赖字节码可确认该默认路径，实际线程增长及生产依赖版本仍须由T04验证。
...
- **措施/验收**：第一轮由[W03](#w03)统筹连接、控制消息、前置查询和恢复准入及前端协同；[W01](#w01)共用实例累计缓冲字节，[W04](#w04)治理计数扫描及锁外清理，[W12](#w12)的WS配额联调与W03同步，[W09](#w09)提供观测。T22/D05分别验证空闲连接、活跃推送、慢消费、高频控制/订阅、并发重连及发布/依赖恢复，覆盖阈值内、阈值边界和超限；证明拒绝/断连可解释、资源不越预算，正常聊天/Stop可用，取消/退出后连接、订阅、timer、缓冲、FD及前置任务回落，故障解除后积压收敛。明确多实例/扩缩容/AZ失效的额度作用域；生产生效配置、容量及真实浏览器证据待取得，未取得前不放行大连接量。
...
| B 资源与阻塞治理 | W01–W06、W08：流/WS连接与恢复/锁与等待、文件、查询和旁路；W12的WS配额联调与W03同步 | 第一轮完成R22准入/累计预算及客户端协同，验收前不放行大连接量；资源和积压有界，正常聊天/Stop/心跳可运行；W06在放开500MiB前完成字节/磁盘/全流生命周期验收 |
| C 任务清理与服务隔离 | W07取消残留；W11共享预算、模块隔离、联合契约和目标拆分 | 真实任务/连接释放，混合负载主链路不被拖累；拆分不能放大共享额度，启用依赖前取得合同和隔离证据 |
| D 入口与容灾 | W12其余入口/静态容灾、W13 ADS/AZ、W14 Region | 先独立静态源/故障域/余量，再验收隔离旧写、恢复点、单写接管与回切 |
...
**关联**：R01、R07，协同R22累计发送预算；应用主责，性能测试协作；T01/T07/T22，RB01/RB02/RB06，D01/D05。
...
- **WS累计预算**：与W03共用实例级发送字节预算，覆盖所有连接队列及已出队未完成发送的数据；记录decorator/协议/网络缓冲的独立上限和测得开销，避免重复计费或漏计。正常、拒绝、断连、超时、取消及退出时幂等归还；在昂贵序列化/复制前先限制事件规模，禁止仅把每连接2MiB乘以连接数当成完整堆预算。慢发送期限须以底层调用退出、线程和socket释放验证，不能只依赖decorator检查。
...
**责任**：应用/DBA共同主责，JVM线程问题由平台协作；R18、R09，协同R22；JVM停滞用RB02，DB等待用RB03，跨服务预算归W11。
...
- **JVM临界区**：记录monitor对象、作用域、持有线程、等待栈和同步回调。WS实例锁内扫描/替换订阅先测耗时；将每用户连接与每topic订阅全表扫描改为原子增减计数，并核对替换、拒绝、重复关闭/注销及异常回滚的一致性，避免计数泄漏或负数。锁内原子更新并剥离句柄、锁外幂等dispose，保留同topic顺序。topic/Relay信号采用保序的串行发射或经验证的锁外方案，保留拒绝/终止/清理语义；不把真实send本已在锁外的Servlet队列改成大锁，不因synchronized存在就删锁。
...
- **监控**：JVM/容器CPU、GC、堆/native/线程/FD/磁盘；各队列深度、字节和年龄、许可/拒绝、DB连接/锁、Redis重连/恢复、遗留Run/Interaction、取消残留及依赖失败/降级负载。R22补WS连接/订阅数、握手/控制消息及重连速率、前置查询在途/拒绝、累计缓冲字节、发送耗时/拒绝/关闭原因、锁持有时长和空闲清理耗时，按实例/AZ及受控租户维度观察，并关联聊天/Stop时延。新指标为待实现，日志不能当作已经存在的Prometheus指标；禁止未经控制使用runId/userId作为指标label。
...
**责任**：WCM、对象存储、网络/ALB、saas gateway及前端共同主责；R11/R12，协同R02/R22；对应[DEP04](deployment.md#dep04)。WS连接/握手配额及拒绝/重连联调在第一轮与W03同步，静态容灾和其余入口工作仍按阶段D推进。
