...
| `runtime.progress` | Relay 原始字段 + `{ "source": "relay", "sourceType": "relay-progress/plan-update/approval-result/..." }` |
...
- `relay-start/relay-progress/relay-end/clarified-query/plan-update/subagent-plan-created/subagent-subtask/approval-result` 映射为 `runtime.progress`；`session-ready/session-state/project-home/available-modes/self-evolution-status/token-update/heartbeat-response` 映射为 `runtime.metadata`；`agent-call` 映射为 `runtime.agent`；`agent-reasoning/thinking-operation-*/thinking-content-update` 映射为 `runtime.thinking`；`tool-call-streaming/tool-execution/tool-structured-result` 映射为 `runtime.tool`；引用/来源类事件映射为 `runtime.reference`；`approval-request` 映射为 `runtime.card`。
- `tool-structured-result` 是 Relay 内部工具调用的结构化结果，本轮不再拆分 `result_data/resultData.widget.data`，统一作为 `runtime.tool` 输出。payload 完整保留 `result_data/resultData/index/total/is_last` 等原字段，前端按 Relay 文档自行解析；`result_data.is_last=true` 不表示本轮 run 完成，run 仍以 `session-state`、连接正常关闭或 `stream-complete/[DONE]` 等终态闭合。
...
stop 请求如果携带 Cookie，后端会按同一规则把 Cookie 透传给可信 Relay 或 DomainAgent 的 cancel adapter，用于下游企业权限校验。Relay stream-http 使用 HTTP stopPath 尽力取消；Relay WebSocket 优先在本机 active WS 上发送 `{"type":"interrupt"}`。如果 stop 请求落到其他实例或本机连接已清理，后端会新建临时 Relay WS，使用 run 中已回填的 Relay `runtimeSessionId` 发送 `config(sessionMode=resume, supports_incremental_recovery=true)`，收到 `session-ready` 后再发送 `interrupt`，然后等待 `session-state=paused` 或 ack 超时后释放临时连接。下一轮提问会重新建立 Relay WS，并通过已有 runtime session 发送 `sessionMode=resume`。即使下游 cancel/interrupt 失败，本服务仍以本地 `run.cancelled` 终态为准。
