...
| `runtime.card` | Relay `approval-request`、`expert_rejection` 或 `skill-card-broadcast` 原始字段，补充 `source=relay` 和对应 `sourceType`；domain-agent 卡片仍按 DomainAgent payload 规则返回 |
...
- Relay `skill-card-broadcast` 每帧映射为一条 `runtime.card`，`payload.source=relay`、`payload.sourceType=skill-card-broadcast`。`cardType=url` 携带 `cardUrl`，`cardType=diyCardScene` 携带 `diyCardScene`；原始 `cardSources/message/min_selections/isSkillDiy`、实例、会话、版本字段及嵌套业务数据按现有脱敏规则保留。前端按 `cardType` 渲染，按事件顺序消费两帧，不拼入 assistant 正文，不因出现选项而进入 Ask User 等待。
- FULL 模式下，技能广播保存为两条独立可见 `CARD` Part（`sourceType=skill-card-broadcast,channel=card,displayHint=inline,visible=true`）。Event Resume 返回同样的 `runtime.card`；新建单轮分享的 `snapshot.parts` 和多消息分享的 `snapshot.messages[].parts` 包含对应卡片和脱敏后的完整 payload。分享页需支持 `url/diyCardScene` 渲染，外部脚本可访问性及交互另行联调。no-store 和敏感信息 Relay 的过滤规则不变；旧隐藏 Part 及已有固定分享快照不自动转换。
...
- DomainAgent独立返回`type=recommended_questions`且`recommendedQuestions`为数组时，映射为`runtime.card`；payload固定包含`sourceType=recommended_questions`、`cardType=recommendedQuestions`、`cardSources=[recommendedQuestions]`以及完整推荐问，并保留下游`conv_id/run_id/seq`。下游`run_id/seq`仅为卡片业务字段，不参与ChatService的runId和事件sequence。`openCard`同帧返回推荐问时继续保持`openCard`卡片分类，数组顺序、嵌套`metadata`及业务扩展字段保持不变。
