...
stop 请求如果携带 Cookie，后端会按同一规则把 Cookie 透传给可信 Relay 或 DomainAgent 的 cancel adapter，用于下游企业权限校验。Relay stream-http 使用 HTTP stopPath 尽力取消；Relay WebSocket 优先在本机 active WS 上发送 `{"type":"interrupt"}`。如果 stop 请求落到其他实例或本机连接已清理，后端会新建临时 Relay WS，发送 `config(sessionMode=resume, supports_incremental_recovery=true)`，收到 `session-ready` 后再发送 `interrupt`，随后释放临时连接。下一轮提问会重新建立 Relay WS，并通过已有 runtime session 发送 `sessionMode=resume`。即使下游 cancel/interrupt 失败，本服务仍以本地 `run.cancelled` 终态为准。
