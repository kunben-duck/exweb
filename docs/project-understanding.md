...
15. `AgentDataPersistenceGate` 在调用 DomainAgent 前按可信skillId读取统一技能配置快照；任何附件都按`attachmentType`校验，空值表示技能不支持上传附件，留存功能开启时同一快照再解析assistant留存策略。Relay和无须配置的请求不查询。批准路由切换时仅使用确认请求显式提交并重新鉴权的附件，不继承原run。附件不支持时不订阅Runtime，只准备未落库的最终技能Binding草稿并输出结构化业务完成事件；草稿仅在`run.completed`终态事务内与Event、assistant和run状态一起原子激活，失败、取消或失权不会改变附件校验前的Binding。正常路径继续按原顺序写入最终路由和run metadata。
...
- Relay 和 domain-agent 过程帧按语义转成 `runtime.progress/runtime.metadata/runtime.agent/runtime.thinking/runtime.tool/runtime.reference/runtime.card`；domain-agent 的 `diyCardScene/openCard/specificSceneInfo/searchList/sourcesDocuments/processResult` 即使跨网络 DataBuffer，也会在单帧硬上限内闭合后输出一个完整语义事件，业务字符串和数组不做静默截断。当前 domain-agent 协议下 `cardUrl/diyCardScene/cardList/openCard/specificSceneInfo` 通常不会在同一个 frame 中同时出现，`runtime.card.payload.sourceType` 会保留原始字段名，例如 `diyCardScene`、`openCard` 或 `specificSceneInfo`；`specificSceneInfo` 对应的可见 `CARD` part 会进入新创建的分享快照，未知完整 JSON 才转成 `runtime.event`。
