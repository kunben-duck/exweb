...
public interface FinanceChatFacade {
     ...
     * 创建并消费一轮聊天事件流。
     *
     * <p>该方法供 {@code start} 在后台执行 run 使用，对外提问入口只暴露
     ...
     */
    Flux<ChatEvent> chat(UserContext user, ChatCommand command);

    /**
     * 后台启动一轮聊天运行，并返回前端订阅所需的 handoff 信息。
     *
     * @param user 请求入口解析出的不可变用户身份快照。
     * @param command 聊天命令，包含会话、用户输入、附件和 metadata。
     * @return 后台 run 的 handoff 信息。
     */
    Mono<ChatRunHandoff> start(UserContext user, ChatCommand command);
     ...
     */
    Mono<ChatRunHandoff> retry(UserContext user, String runId, ChatCommand command);
     ...
     */
    Mono<ChatRunStopResult> stop(UserContext user, String runId);
}
