...
/**
 * SuperAgent 侧维护的 Relay Runtime 会话绑定。
 *
 ...
 * @param provider Runtime provider 编码，正式版本固定为 relay-agent。
 * @param runtimeSessionId Relay Runtime 返回的内部会话标识，首次调用可为空。
 * @param status 绑定状态，只用于判断是否继续路由到 Relay Runtime。
...
public record RuntimeBinding(
     ...
     *
     * @param now 当前时间。
     * @return true 表示本轮请求应继续进入 Relay Runtime。
     */
