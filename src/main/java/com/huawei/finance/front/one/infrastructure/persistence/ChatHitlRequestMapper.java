...
/**
 * fin_ex_chat_hitl_request_t 的 MyBatis Mapper。
 */
@Mapper
public interface ChatHitlRequestMapper {
    /**
     * 创建 HITL 等待请求。
     *
     ...
     * @return 影响行数。
     */
    int insert(ChatHitlRequestRow row);

    /**
     * 按 owner 和请求 ID 查询 HITL 请求。
     *
     * @param tenantId 租户标识。
     * @param userId 用户标识。
     ...
     */
    ChatHitlRequestRow findByOwnerAndId(@Param("tenantId") String tenantId,
                                        @Param("userId") String userId,
                                        @Param("hitlRequestId") String hitlRequestId);
     ...
     */
    ChatHitlRequestRow findWaitingBySession(@Param("tenantId") String tenantId,
                                            @Param("userId") String userId,
     ...
     *
     * @param row claim 更新行，包含 owner、hitlRequestId、continueRunId 和响应 payload。
     * @return 影响行数。
     */
    int claimForResponse(ChatHitlClaimRow row);
