...
import com.huawei.finance.front.one.interfaces.chat.dto.FrontMessageFeedbackDto;
import com.huawei.finance.front.one.interfaces.chat.dto.FrontMessageFeedbackRequest;
import com.huawei.finance.front.one.interfaces.chat.dto.FrontChatRequest;
import com.huawei.finance.front.one.interfaces.chat.dto.FrontChatRunHandoffDto;
import com.huawei.finance.front.one.interfaces.chat.dto.FrontChatStopResultDto;
...
public class ChatController {
     ...
     * 不随每次 run handoff 返回，避免后端业务响应承担客户端路由配置职责。</p>
     *
     * @param request 前端提问请求，只包含会话、用户文本、附件引用和 metadata；租户与用户由服务端身份上下文解析。
     * @return 新建后台 run 的 handoff 信息，包含 runId、sessionId、firstSeq 和 streamTopicId。
     */
    @PostMapping(value = "/runs")
    public Mono<FrontChatRunHandoffDto> startRun(@RequestBody FrontChatRequest request) {
        UserContext user = resolveChatUser();
        return chatFacade.start(user, requestTranslator.toCommand(request))
                .map(handoff -> new FrontChatRunHandoffDto(
                        handoff.runId(),
                        handoff.sessionId(),
                        handoff.firstSeq(),
                        handoff.createdAt(),
                        handoff.streamTopicId()
                ));
    }
    ...
    public Mono<FrontChatStopResultDto> stopRun(@PathVariable String runId) {
        UserContext user = resolveChatUser();
        return chatFacade.stop(user, runId)
                .map(this::toStopDto)
     ...
     * @return 新 run 的 handoff 信息。
     */
    @PostMapping(value = "/runs/{runId}/retry")
    public Mono<FrontChatRunHandoffDto> retryRun(@PathVariable String runId,
                                                 @RequestBody(required = false) RetryChatRunRequest request) {
        UserContext user = resolveChatUser();
        return chatFacade.retry(user, runId, requestTranslator.toRetryCommand(request))
                .map(handoff -> new FrontChatRunHandoffDto(
                        handoff.runId(),
                        handoff.sessionId(),
                        handoff.firstSeq(),
    ...
    public Mono<FrontMessageFeedbackDto> feedback(@PathVariable String messageId,
                                                  @RequestBody FrontMessageFeedbackRequest request) {
        UserContext user = resolveChatUser();
        return Mono.fromCallable(() -> {
                    ...
                    return new FrontMessageFeedbackDto(feedback.id(), feedback.messageId(), feedback.runId(),
     ...
     * @return SSE 事件流，event name 等于聊天事件 type，data 为 FrontChatEventDto。
     */
    @GetMapping(value = "/sessions/{sessionId}/events/sse", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
    public Flux<ServerSentEvent<FrontChatEventDto>> resumeSse(@PathVariable String sessionId,
    ...
    public Mono<FrontStreamStatusDto> streamStatus(@PathVariable String sessionId) {
        UserContext user = resolveChatUser();
    ...
    private FrontChatStopResultDto toStopDto(ChatRunStopResult result) {
        return new FrontChatStopResultDto(
                result.runId(),
                result.sessionId(),
                result.status().name(),
                result.latestSeq(),
