...
/** Owns background subscription and first persisted-event handoff for newly started runs. */
final class ChatRunStartCoordinator {
    ...
    Mono<ChatRunStartResult> startInteraction(UserContext user,
                                              TraceContext traceContext,
    ...
    private void subscribeInteraction(Flux<ChatEvent> runFlux,
                                      BackgroundStartState state,
                                      TraceContext traceContext,
                                      String interactionId) {
        Disposable disposable = runFlux.subscribe(
    ...
    void trackRun(RunStartAttempt attempt, ChatRun run, String stage) {
        if (attempt == null) {
            return;
        }
    ...
    void trackExecution(RunStartAttempt attempt, RunExecutionClaim executionClaim, String stage) {
        if (attempt != null) {
    ...
    void ensureActive(RunStartAttempt attempt, String stage) {
        if (attempt == null || !attempt.aborted()) {
            return;
        }
    ...
    private RuntimeException rejected(RunStartAttempt attempt, String stage) {
        return new com.huawei.it.ex.one.application.integration.conversation.ChatEventAppendRejectedException(
                "run start attempt 已在首事件交接前终止: runId=" + attempt.runId() + ", stage=" + stage);
    }

    @FunctionalInterface
    interface StandardRunFactory {
        Flux<ChatEvent> create(RunStartAttempt attempt);
    }

    @FunctionalInterface
    interface InteractionRunFactory {
