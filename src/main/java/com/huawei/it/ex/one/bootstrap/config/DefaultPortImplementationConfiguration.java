...
import com.huawei.it.ex.one.runtime.infrastructure.UnsupportedAgentRuntimeInteraction;
import com.huawei.it.ex.one.runtime.infrastructure.UnsupportedAgentRuntimeRecoveryPort;
import com.huawei.it.ex.one.intent.infrastructure.runtime.NoopIntentAgentRuntime;
...
public class DefaultPortImplementationConfiguration {
     ...
     * resume token 和幂等输出保证，提供新的 {@link RuntimeRecoveryService} bean 即可覆盖。</p>
     *
     * @return 不支持断点接管的默认 Runtime recovery port。
     */
    @Bean
    @ConditionalOnMissingBean(RuntimeRecoveryService.class)
    public RuntimeRecoveryService unsupportedAgentRuntimeRecoveryPort() {
