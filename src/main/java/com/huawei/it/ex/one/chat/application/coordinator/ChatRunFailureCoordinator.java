...
/** Preserves existing execution-initialization and post-creation failure closure behavior. */
@Component
public class ChatRunFailureCoordinator {
    ...
    public ChatRunFailureCoordinator(ChatRunTerminalCommitService terminalCommitService,
    ...
    public Flux<ChatEvent> failInteractionContinuation(RunEventPipelineContext context,
        ...
        return eventPersistenceCoordinator.persistAndPublish(
                        Flux.just(runtimeErrorEvent(context.runId(), context.session().id(), failure)), context)
                .doFinally(ignored -> runExecutionRegistry.complete(context.executionClaim()));
    }

    public Flux<ChatEvent> failExecutionInitialization(ChatRun run,
    ...
    public ErrorEvent runtimeErrorEvent(String runId, String sessionId, Throwable failure) {
