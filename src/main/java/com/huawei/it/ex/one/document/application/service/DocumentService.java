...
public interface DocumentService {
     ...
     */
    List<ResolvedDocumentAttachment> resolveAttachmentsForUser(
     ...
     * 因此不能只依赖 {@link ResolvedDocumentAttachment} 的展示字段。</p>
     *
     * @param user 当前调用方身份快照。
     * @param attachments 前端传入的附件引用。
     * @return 已完成归属和状态校验的文档元数据。
     */
    List<UploadedDocument> resolveDocumentsForUser(
            UserContext user, List<DocumentAttachmentRequest> attachments);
     ...
     */
    ResolvedDocumentAttachments resolveChatAttachmentsForUser(
            UserContext user, List<DocumentAttachmentRequest> attachments);
