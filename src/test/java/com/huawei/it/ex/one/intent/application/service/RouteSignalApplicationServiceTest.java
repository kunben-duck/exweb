...
class RouteSignalApplicationServiceTest {
    ...
    void disabledSignalsRouteInitialToRuntimeWithoutCallingClients() {
        ...
        RouteSignalResult result = routeInitial(service, user, session, command, List.of(), memory);
    ...
    void selectedIntentPresentationMetadataIsNotSentToUseCaseLibrary() {
        ...
        RouteSignalResult result = routeInitial(service, user, session, selectedCommand, List.of(), memory);
    ...
    void intentQueryOverridesOnlyIntentCommandMessage() {
        AtomicReference<IntentCommandSnapshot> capturedIntentCommand = new AtomicReference<>();
        RouteSignalApplicationService service = service(false, true,
                request -> UseCaseMatchResult.notMatched("disabled"),
        ...
        RouteSignalResult result = service.routeInitialWithProgress(ChatIntentMapper.toRouteRequest(
    ...
    void routeInitialWithProgressEmitsIntentAgentEventsBeforeRouteResult() {
        ...
        StepVerifier.create(service.routeInitialWithProgress(ChatIntentMapper.toRouteRequest(
                        new ChatIntentMapper.RouteRequestInput(
                                "run1", user, session, command, List.of(), memory, command.message()))), 1)
                .assertNext(frame -> {
    ...
    void sameRunInlineRouteIsMergedWithoutWaitingForRouteMemoryWrite() {
        ...
        RouteSignalResult result = routeInitial(service, user, session, command, List.of(), inlineMemory);
    ...
    void topLevelUserCorrectionTriggerWinsOverMetadataAndFallback() {
        ...
        ChatCommand correction = new ChatCommand("cmd2", "tenant1", "user1", "session1",
                ...
                null, null, com.huawei.it.ex.one.chat.domain.ChatRunMode.NEXT,
                null, null, null, "user_correction");

        RouteSignalResult result = routeInitial(service, user, session, correction, List.of(), memory);
    ...
    void intentWaitingClarificationStopsAtWaitingRoute() {
        IntentService intentService = new IntentService() {
            @Override
            public IntentDecision recognize(IntentCommandSnapshot command, MemoryContext memory, UserContext user) {
                throw new AssertionError("recognizeForRouting should be used");
            }

            @Override
            public IntentRecognitionResult recognizeForRouting(IntentCommandSnapshot command, MemoryContext memory, UserContext user) {
    ...
    void clarifyAnswerUsesInlineHistoryWhenRouteMemoryIsUnavailable() {
        ...
        RouteSignalResult result = routeInitial(service, user, session, clarifyAnswer, List.of(), memory);
    ...
    void failRunStrategyReturnsFailureResultWithoutRuntimeRoute() {
        ...
        List<RouteSignalFrame> frames = service.routeInitialWithProgress(ChatIntentMapper.toRouteRequest(
                new ChatIntentMapper.RouteRequestInput(
                        "run1", user, session, command, List.of(), memory, command.message())))
                .collectList().block();
    ...
    private RouteSignalResult routeInitial(
            RouteSignalApplicationService service,
            UserContext user,
            ChatSession session,
            ChatCommand command,
            List<AttachmentRef> attachments,
            MemoryContext memory) {
        return service.routeInitial(ChatIntentMapper.toRouteRequest(
                new ChatIntentMapper.RouteRequestInput(
                        null, user, session, command, attachments, memory,
